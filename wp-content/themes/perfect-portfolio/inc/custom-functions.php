...
function perfect_portfolio_change_comment_form_default_fields( $fields ){    
    ...
    $fields['author'] = '<p class="comment-form-author"><label class="screen-reader-text" for="author">' . esc_html__( 'Name*', 'perfect-portfolio' ) . '</label><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'perfect-portfolio' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><label class="screen-reader-text" for="email">' . esc_html__( 'Email*', 'perfect-portfolio' ) . '</label><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'perfect-portfolio' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
...
function perfect_portfolio_change_comment_form_defaults( $fields ){ 
    ...
    $fields['comment'] = '<p class="comment-form-comment"><label class="screen-reader-text" for="comment">' . esc_html__( 'Comment', 'perfect-portfolio' ) . '</label><textarea id="comment" name="comment" cols="40" rows="8" required="required" placeholder="' . esc_attr__( 'Comment','perfect-portfolio' ) . '"></textarea></p>';;
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p><?php printf( __( '%1$s is now installed and ready to use. Click below to see theme documentation, plugins to install and other details to get started.', 'perfect-portfolio' ), $name ) ; ?></p>
...
if( ! function_exists( 'perfect_portfolio_restore_admin_notice' ) ) :
/**
 * Restoring admin notice on theme switch
 */
function perfect_portfolio_restore_admin_notice(){
    update_option( 'perfect_portfolio_admin_notice', false );
}
endif;
add_action( 'after_switch_theme', 'perfect_portfolio_restore_admin_notice' );