...
if( ! class_exists( 'PF_Field_spacing' ) ) {
  class PF_Field_spacing extends PF_classFields {
     ...
     * Render field
     *
     * @since   1.4.6   2019-06-12  Validation in the unit and show_unit
     * @since   1.6     2019-12-02  New 'is-unit' class validation is added for better interpretation
     *
     * @return  string
     */
    public function render() {
      ...
      if( ! empty( $args['all'] ) ) {
        ...
        echo '<input type="number" name="'. $this->field_name('[all]') .'" value="'. $value['all'] .'"'. $placeholder .' class="pf-input-number'. $is_unit .'" />';
        echo ( $unit ) ? '<span class="pf--label pf--unit">'. $args['units'][0] .'</span>' : '';
        echo '</div>';
        ...
        foreach( $properties as $property ) {
          ...
          echo '<input type="number" name="'. $this->field_name('['. $property .']') .'" value="'. $value[$property] .'"'. $placeholder .' class="pf-input-number'. $is_unit .'" />';
          echo ( $unit ) ? '<span class="pf--label pf--unit">'. $args['units'][0] .'</span>' : '';
          echo '</div>';
